...
function setup() {
  ...
  colorPool.add(color(60, 100, 60));
  colorPool.add(color(0, 100, 60));
  colorPool.add(color(20, 100, 60));
  colorPool.add(color(40, 100, 60));
  colorPool.add(color(80, 100, 60));
  ...
  for (var i = 0; i < Math.floor(height / grid_size); i ++) {
    for (var j = 0; j < Math.floor(width / grid_size); j ++) {
      ...
      if (Math.random() > 0.5) {
        fill(colorPool.random());
        drawBar(j * grid_size, i * grid_size, grid_size / Math.cos(PI / 4) + bar_width, bar_width, PI / 4);
      }
      else {
        fill(colorPool.random());
        drawBar(j * grid_size, i * grid_size, grid_size / Math.cos(PI / 4) + bar_width, bar_width, -PI / 4);
